...
public class MD5Loader {
    ...
    public static AnimatedGameItem process(MD5Model md5Model, MD5AnimationModel animModel, Vector4f defaultColour) throws Exception {
        ...
        AnimatedGameItem result = new AnimatedGameItem(meshes, animatedFrames, invJointMatrices);
        return result;
    }
    ...
    private static Mesh generateMesh(MD5Model md5Model, MD5Mesh md5Mesh) {
        ...
        List<MD5Vertex> md5Vertices = md5Mesh.getVertices();
        List<MD5Weight> weights = md5Mesh.getWeights();
        List<MD5JointData> joints = md5Model.getJointInfo().getJoints();

        for (MD5Vertex md5Vertex : md5Vertices) {
    ...
    private static AnimatedFrame processAnimationFrame(MD5Model md5Model, MD5AnimationModel animModel, MD5Frame frame, List<Matrix4f> invJointMatrices) {
        ...
        List<MD5HierarchyData> hierarchyList = animModel.getHierarchy().getHierarchyDataList();

        List<MD5JointData> joints = md5Model.getJointInfo().getJoints();
        int numJoints = joints.size();
        float[] frameData = frame.getFrameData();
        for (int i = 0; i < numJoints; i++) {
            MD5JointData joint = joints.get(i);
            MD5BaseFrameData baseFrameData = baseFrame.getFrameDataList().get(i);
